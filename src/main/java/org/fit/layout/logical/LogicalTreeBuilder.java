...
public class LogicalTreeBuilder extends BaseLogicalTreeProvider
{
    ...
    private PresentationLogicalTree tree;
    ...
    public LogicalAreaTree createLogicalTree(AreaTree areaTree)
    {
        ...
        tree = new PresentationLogicalTree(areaTree);
    ...
    private LogicalArea createRoot(AreaTree tree)
    {
        ...
        return new LogicalArea(tree.getRoot(), uri, tagRoot);
    }
    ...
    private void addVTitle()
    {
        ...
        rootArea.appendChild(new LogicalArea(root, text, tagVTitle));
        curtitle = text;
    }
    ...
    private void addIndexData()
    {
        ...
        if (matcher.find())
        {
            ...
            rootArea.appendChild(new LogicalArea(leaves.elementAt(iTitle), curtitle, tagVTitle));
            rootArea.appendChild(new LogicalArea(leaves.elementAt(iTitle), dfmt.format(IndexFile.getPubDate(vol)), tagPublished));
            List<Integer> related = IndexFile.getRelated(vol);
            if (related != null)
            {
                for (int rel : related) 
                    rootArea.appendChild(new LogicalArea(leaves.elementAt(iTitle), "http://ceur-ws.org/Vol-" + rel + "/", tagRelated));
            }
        }
    }
    ...
    private void analyzeShortNames()
    {
        ...
        for (Event ev : ws)
        {
            LogicalArea sa = new LogicalArea(subtitle, ev.sname, tagVShort);
            rootArea.appendChild(sa);
            if (ev.order > 0)
                sa.appendChild(new LogicalArea(subtitle, String.valueOf(ev.order), tagVOrder));
            String wt = curtitle;
            ...
            sa.appendChild(new LogicalArea(subtitle, wt, tagWTitle));
        }
        if (sp.getColocEvent() != null)
        {
            LogicalArea sa = new LogicalArea(subtitle, sp.getColocEvent().sname, tagColoc);
            rootArea.appendChild(sa);
        }
    ...
    private void addVDates()
    {
        if (iDates != -1)
        {
            ...
            if (dates.size() == 2)
            {
                ...
                rootArea.appendChild(new LogicalArea(a, dfmt.format(dates.get(0)), tagStartDate));
                rootArea.appendChild(new LogicalArea(a, dfmt.format(dates.get(1)), tagEndDate));
            }
            else
    ...
    private void addVCountry()
    {
        if (iCountry != -1)
        {
            ...
            if (countries.size() >= 1)
            {
                ...
                rootArea.appendChild(new LogicalArea(a, uri, tagVCountry));
            }
            if (countries.size() != 1)
    ...
    private List<String> saveEditor(Area editor, String text)
    {
        ...
        if (i > 0)
        {
            ...
            {
                LogicalArea aname = new LogicalArea(editor, curname.trim(), tagVEditor);
                ret.add(curname.trim());
                rootArea.appendChild(aname);
                LogicalArea aaffil = new LogicalArea(editor, affil.trim(), tagEAffil);
                aname.appendChild(aaffil);
                if (affs[1] != null)
                {
                    LogicalArea acountry = new LogicalArea(editor, affs[1], tagECountry);
                    aname.appendChild(acountry);
                }
                if (affsect != null)
                {
                    aname.appendChild(new LogicalArea(editor, affsect, tagSection));
                }
            }
        }
        else
    ...
    private void savePaper(Area title, Area authors, Area pages, Area curSection)
    {
        if (title != null && authors != null)
        {
            ...
            if (pid != null)
            {
                LogicalArea ap = new LogicalArea(title, pid, tagPaper);
                rootArea.appendChild(ap);
                
                LogicalArea at = new LogicalArea(title, title.getText().trim(), tagTitle);
                ap.appendChild(at);
                ...
                for (String name : names)
                {
                    ...
                    {
                        LogicalArea aa = new LogicalArea(authors, nname, tagAuthor);
                        ap.appendChild(aa);
                    }
                }
                if (pages != null)
                {
                    ...
                    if (pp.length == 2)
                    {
                        try {
                            ...
                            LogicalArea ps = new LogicalArea(pages, pp[0], tagStartPage);
                            ap.appendChild(ps);
                            LogicalArea pe = new LogicalArea(pages, pp[1], tagEndPage);
                            ap.appendChild(pe);
                            LogicalArea pn = new LogicalArea(pages, String.valueOf(pi2 - pi1 + 1), tagPages);
                            ap.appendChild(pn);
                    ...
                    else if (pp.length == 1)
                    {
                        try {
                            ...
                            LogicalArea ps = new LogicalArea(pages, pp[0], tagStartPage);
                            ap.appendChild(ps);
                            LogicalArea pe = new LogicalArea(pages, pp[0], tagEndPage);
                            ap.appendChild(pe);
                            LogicalArea pn = new LogicalArea(pages, "1", tagPages);
                            ap.appendChild(pn);
                        } catch (NumberFormatException e) {
                            log.warn("Invalid page numbers: {}", pages);
                        }
                ...
                if (curSection != null)
                {
                    LogicalArea asec = new LogicalArea(curSection, curSection.getText().trim(), tagSection);
                    ap.appendChild(asec);
                }
            }
            else
